...
import { compareYamlObjects } from '../utils/schemaDiff/compareSchemas.ts';
import { renderSchemaDiffComment } from '../utils/schemaDiff/renderComment.ts';
...
function parseYamlOrThrow(content: string, sourceLabel: string): OpenApiDocument {
  try {
...
function listSchemaFiles(localDir: string): string[] {
...
type ParsedFlag =
  { match: false } | { match: true; needsNext: true } | { match: true; needsNext?: false; value: string };

function parseFlag(arg: string, flag: string): ParsedFlag {
  if (arg === flag) {
...
export async function runSchemaDiffPublished(options: RunOptions = {}, deps: RunDeps = {}) {
